...
- 行/浮层的 ☆ 是**单向「添加副本」动作**:把当前生效配置(模型+引擎+深度+Fast)拷进收藏区,点亮 0.7s 反馈后恢复;源头行不持有收藏态(多副本下不可判定)。重复添加去重。**例外**：最近视图的行（§1.2）是配置副本的流水，星标按同一条目已匹配到的收藏亮/灭并可点星取消（点星=删那条收藏，删除口径与收藏视图一致）；普通模型行与收藏行的上述契约不变。
